...
- 列表或数组，其长度与待分组的轴一样(0 代表一行，1代表一列)。
- 表示DataFrame某个列名的值。
...
```python
In [31]: df.groupby(['key1', 'key2'])[['data2']].mean() # 有两个括号产生的结果是dataFrame的形式
...
```python
In [32]: s_grouped = df.groupby(['key1', 'key2'])['data2'] # 有一个括号产生的结果是 series 的形式
...
```python
In [50]: hier_df.groupby(level='cty', axis=1).count() # axis = 1 可以用来表示固定所有的行
Out[50]: 
cty  JP  US
0     2   3
1     2   3
...
聚合指的是**任何能够从数组产生标量值的数据转换过程**。之前的例子已经用过一些，比如mean、count、min以及sum等。你可能想知道在GroupBy对象上调用mean()时究竟发生了什么。许多常见的聚合运算（如表10-1所示）都有进行优化。然而，除了这些方法，你还可以使用其它的。
...
```python
In [64]: grouped_pct.agg([('foo', 'mean'), ('bar', np.std)]) # 可以用其它符号表示列名
Out[64]: 
                  foo       bar
day  smoker                    
...
In [66]: result = grouped['tip_pct', 'total_bill'].agg(functions) # 可以同时选择多列数据进行聚合运算
...
```python
In [73]: tips.groupby(['day', 'smoker'], as_index=False).mean() # 去除index
Out[73]: 
...
```python
In [77]: tips.groupby(['smoker', 'day']).apply(top, n=1, column='total_bill') # n = 1 表示total_bill 中总花费最高的即可。
Out[77]: 
                 total_bill    tip smoker   day    time  size   tip_pct
smoker day                                                             
...
In [83]: quartiles = pd.cut(frame.data1, 4) # 分成四个区间
...
这些都是长度相等的桶。要根据样本分位数得到大小相等的桶，使用qcut即可。传入labels=False即可**只获取分位数的编号**：
```python
# Return quantile numbers
In [88]: grouping = pd.qcut(frame.data1, 10, labels=False) # 先使用data1建立桶

In [89]: grouped = frame.data2.groupby(grouping) #再将桶运用到data2中
...
In [105]: fill_func = lambda g: g.fillna(fill_values[g.name]) # 将遗失值填入一个固定的数
，name属性是分组的名字
...
还可以对这个表作进一步的处理，**传入margins=True可以，添加分项小计**。这将会添加标签为All的行和列，其值对应于单个等级中所有数据的分组统计：
```python
