...
MultiIndex(levels=[['a', 'b', 'c', 'd'], [1, 2, 3]],
           labels=[[0, 0, 0, 1, 1, 2, 2, 3, 3], [0, 1, 2, 0, 2, 0, 1, 1, 2]]) # 上下是一一对应的
```
...
```python
In [16]: data.unstack() # 将内标签打平，将内标签变成列
Out[16]: 
          1         2         3
...
```python
In [54]: pd.merge(left, right, on='key1') # 这里通过 key1 关键字进行合并，对于相同的key2 关键字，需要进行加上后缀
Out[54]: 
...
In [60]: pd.merge(left1, right1, left_on='key', right_index=True) # 这里将左边的key 这一列和右边的 index进行连接
Out[60]: 
  key  value  group_val
0   a      0        3.5
2   a      2        3.5
...
```python
In [66]: pd.merge(lefth, righth, left_on=['key1', 'key2'], right_index=True) # 将左边的两列和右边的两index进行合并
Out[66]: 
...
In [90]: pd.concat([s1, s4], axis=1, join='inner') # inner 可以用来取出两者都有的部分
Out[90]: 
   0  1
a  0  0
b  1  1
...
```python
In [91]: pd.concat([s1, s4], axis=1, join_axes=[['a', 'c', 'b', 'e']]) # 可以改变轴的名称
Out[91]: 
     0    1
a  0.0  0.0
c  NaN  NaN
...
In [94]: result.unstack() # 将最内层的标签进行展开
Out[94]: 
         a    b    f    g
one    0.0  1.0  NaN  NaN
two    0.0  1.0  NaN  NaN
...
此外还有两个用于管理层次化索引创建方式的参数（参见表8-3）。举个例子，我们可以用**names参数命名创建的轴级别**：
```python
...
In [113]: np.where(pd.isnull(a), b, a) # 用b 在该位置的值，来填补a的null
...
```python
In [125]: result.unstack(0) # 最内层标签，传入的参数是0
Out[125]: 
state   Ohio  Colorado
number                
one        0         3
...
In [134]: data2.unstack().stack(dropna=False) # 使用这个属性可以使nan不消失
Out[134]: 
one  a    0.0
     b    1.0
     c    2.0
...
In [141]: periods = pd.PeriodIndex(year=data.year, quarter=data.quarter,
   .....:                          name='date') # 将year 和 quarter 进行合并，叫做date
