...
spx.plot(ax=ax, style='k-') # 用数据进行画图

crisis_data = [ # 列出需要标注的注解
    (datetime(2007, 10, 11), 'Peak of bull market'),
    (datetime(2008, 3, 12), 'Bear Stearns Fails'),
    (datetime(2008, 9, 15), 'Lehman Bankruptcy')
]

for date, label in crisis_data: # 遍历需要标注的注解进行标注
...
ax.set_xlim(['1/1/2007', '1/1/2011']) # 设置显示的区间的范围
...
再以本书前面用过的那个有关小费的数据集为例，假设我们想要做一张堆积柱状图以展示每天各种聚会规模的数据点的百分比。我用read_csv将数据加载进来，然后**根据日期和聚会规模创建一张交叉表**：
...
注意：一般编号为0的是y轴，编号为1的是x轴

```python
# Normalize to sum to 1
In [79]: party_pcts = party_counts.div(party_counts.sum(1), axis=0) # 按照x轴进行求和，计算每一个所占比例
In [80]: party_pcts
Out[80]: 
size         2         3         4         5
day                                         
...
In [99]: sns.distplot(values, bins=100, color='k') # 可以同时画直方图和密度图
```
...
In [102]: trans_data = np.log(data).diff().dropna() # 先对所有的数取log，再进行差分运算

In [103]: trans_data[-5:] #取最后5条数据
Out[103]: 
