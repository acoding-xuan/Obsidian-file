...
字典的各列可以使用不同的NA标记值：
```python
In [31]: sentinels = {'message': ['foo', 'NA'], 'something': ['two']} # 将 message 中的 foo 和 NA 变为 NaN

In [32]: pd.read_csv('examples/ex5.csv', na_values=sentinels) # 将字典指定的行或列，当成NaN
Out[32]:
something  a   b     c   d message
0       one  1   2   3.0   4     NaN
...
tot = pd.Series([]) 
for piece in chunker: # 统计所有的块中，所有的每一个‘key’ 出现的次数
...
Series也有一个to_csv方法：
```python
In [50]: dates = pd.date_range('1/1/2000', periods=7) #从 2000 -01 - 01 向后取七天
...
```python
In [57]: with open('examples/ex7.csv') as f: # with open 可以自动关闭
...
![image-20230804201426742](C:\Users\86155\AppData\Roaming\Typora\typora-user-images\image-20230804201426742.png)

![image-20230804201800008](C:\Users\86155\AppData\Roaming\Typora\typora-user-images\image-20230804201800008.png)
...
with open('mydata.csv', 'w') as f:
    writer = csv.writer(f, dialect=my_dialect) # 这里可以使用自己定义的 dialect 
...
![image-20230804204733453](C:\Users\86155\AppData\Roaming\Typora\typora-user-images\image-20230804204733453.png)

```python
In [71]: print(data.to_json())
{"a":{"0":1,"1":4,"2":7},"b":{"0":2,"1":5,"2":8},"c":{"0":3,"1":6,"2":9}}

In [72]: print(data.to_json(orient='records')) # 自己查 orient 可以用来 改变数据的表示方式
...
HDF5是一种存储大规模科学数组数据的非常好的文件格式。它可以被作为C标准库，带有许多语言的接口，如Java、Python和MATLAB等。HDF5中的**HDF指的是层次型数据格式（hierarchical data format）**。每个HDF5文件都含有一个文件系统式的节点结构，它使你能够存储多个数据集并支持元数据。与其他简单格式相比，HDF5支持多种压缩器的即时压缩，还能更高效地存储重复模式数据。对于那些非常大的无法直接放入内存的数据集，HDF5就是不错的选择，因为它可以高效地分块读写。
...
File path: mydata.h5
/obj1                frame        (shape->[100,1])  # 以字典的形式构成                             
...
HDFStore支持两种存储模式，'fixed'和'table'。后者通常会更慢，但是**支持使用特殊语法进行查询操作**：
```python
...
```python
In [101]: frame.to_hdf('mydata.h5', 'obj3', format='table') # 以hdf的形式进行储存
