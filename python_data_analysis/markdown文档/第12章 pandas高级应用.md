...
如果你**已经从其它源获得了分类编码**，你还可以使用**from_codes**构造器：
```python
...
输**出[foo < bar < baz]指明‘foo’位于‘bar’的前面，以此类推**。无序的分类实例可以通过as_ordered排序：
```python
...
```python
In [65]: actual_categories = ['a', 'b', 'c', 'd', 'e'] # 这里多加了一个 'e'
...
在大数据集中，分类经常作为节省内存和高性能的便捷工具。过滤完大DataFrame或Series之后，许多分类可能不会出现在数据中。我们可以**使用remove_unused_categories方法删除没看到的分类**：
```python
In [70]: cat_s3 = cat_s[cat_s.isin(['a', 'b'])] # 取出 a 和 b的部分
...
在第10章，我们在分组操作中学习了apply方法，进行转换。还有另一个**transform**方法，它与apply很像，但是对使用的函数有一定限制：
